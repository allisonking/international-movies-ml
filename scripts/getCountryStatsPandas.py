...
def main():
    ...
    input_file = pd.read_csv('movie-countries.csv', keep_default_na=False)
    ...
    # write to CSV
    sorted_result.to_csv("movie-countries-pandas.csv", index=False)

    axes = sorted_result.plot(x='country', y='primary_count', kind='bar', title='Number of Movies from a Country',
                              legend=False)
